...
maxLengthInput = function() {
	var config = {
		...
		max_length = function(nodes) {
			...
			for (var i = 0, len = nodes.length; i < len; i += 1) {
				...
				nodes[i].addEventListener("focus", function(event) {
					...
					counter.style.left = event.currentTarget.offsetLeft + event.currentTarget.offsetWidth - counter.offsetWidth - 10;
		...
		retrieve_args = function(c) {
			...
			if (c.limit_class && typeof c.limit_class === "string") {
				config.limit_class = c.limit_class;
			}

			if (c.interrupt_after_threshold !== undefined && typeof c.interrupt_after_threshold === "boolean") {
				config.interrupt_after_threshold = c.interrupt_after_threshold;
			}

			if (c.asc !== undefined && typeof c.asc === "boolean") {
				config.asc = c.asc;
			}
		},
		arg = arguments;
